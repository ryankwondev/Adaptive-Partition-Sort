...
def run(datas):
    ...
    df = pd.DataFrame(columns=['Algorithm', 'Best Memory', 'Worst Memory', 'Average Memory'])  # B
    ...
    aps_best, aps_worst, aps_avg = get_mean(lambda data: adaptive_partition_sort(data, 0), datas)
